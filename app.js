  ...
  function encodePixels(imageData, digits) {
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < leftEnd; x += 1) {
        ...
        for (let c = 0; c < 3; c += 1) {
          data[offset + c] = wrapPixelValue(data[offset + c] + keys.left);
        }
      }
      ...
      for (let x = leftEnd; x < middleEnd; x += 1) {
        const offset = pixelOffset(x, y, width);
        for (let c = 0; c < 3; c += 1) {
          data[offset + c] = wrapPixelValue(data[offset + c] + keys.middle[c]);
        }
      }
      ...
      for (let x = middleEnd; x < width; x += 1) {
        const offset = pixelOffset(x, y, width);
        for (let c = 0; c < 3; c += 1) {
          data[offset + c] = wrapPixelValue(data[offset + c] + keys.right);
        }
      }
    }
